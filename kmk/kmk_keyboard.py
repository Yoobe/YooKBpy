...
from kmk.hid import USBHID, HIDModes, BLEHID, AbstractHID
...
class KMKKeyboard:
    ...
    def YooKBInit(self, hid_type=HIDModes.USB, secondary_hid_type=None, **kwargs) -> None:
        self._init(hid_type=hid_type, secondary_hid_type=secondary_hid_type, **kwargs)

    def YooKBmain(self) -> bool:
        # debug('YooKBmain')
        # try:
        # debug('YooKBmain - loop ', self)
        self._main_loop()
        return True
        # finally:
        #     debug('YooKBmain Unexpected error: cleaning up')
        #     self._deinit_hid()
        #     self.deinit()
        #     return False
